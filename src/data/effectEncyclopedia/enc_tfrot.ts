...
export const tfRotEnc: Encyclopedia = {
  'tf-rotate': {
    artGuide: `rotate 是绕 anchor（默认中心）的 2D 平面旋转，角度以度计（正为顺时针，从正上起算）。它是做自转装饰、强调甩头、技能 CD 环、转场旋转元素的底层属性。
动画 rotate 从 0 到 360 即一圈自转；配合 rotate_pad 可避免旋转时尺寸抖动。`,
    paramManual: [
...
rotate_pad=True 用正方形外接框包裹，尺寸恒定但可能露出更多透明边距。`,
  ...
  'tf-rotate-pad': {
    artGuide: `rotate_pad 为旋转后的显示件补足成「正方形外接框」，使其在旋转全程尺寸恒定、不忽大忽小。固定角度旋转时（如永远 45°）可设为 False 取最小包围盒，省下边距。
它是一个「尺寸稳定性」开关，与视觉无关，只影响包围盒。`,
    paramManual: [
...
设 False 后若旋转角度接近 45° 且立绘接近方形，可能明显可见尺寸「呼吸」。`,
  ...
  'tf-transform-anchor': {
    artGuide: `transform_anchor 让锚点落在「被裁剪后的子图」上，并随缩放/旋转一起移动——等于把锚点变成立绘真正的「旋转缩放中心」。
当你带 crop 做变换、或希望严格绕自身真实中心旋转时，开它最稳，避免「转着转着偏心」。`,
    paramManual: [
...
纯未裁剪的整图，默认 anchor 与 transform_anchor 效果几乎一致，无需额外开。`,
  ...
  'tf-zoom': {
    artGuide: `zoom 是整体等比缩放因子（1.0 为原大），宽高同乘、保持比例。推近强调、物体放大缩小、呼吸式胀缩、镜头 zoom 模拟，都靠它。
它与 xzoom/yzoom 不同：zoom 强制等比，后者可非等比。`,
    paramManual: [
...
要与 rotate 叠加时，注意两者都绕 anchor，顺序/锚点一致才能预期。`,
  ...
  'tf-xzoom': {
    artGuide: `xzoom / yzoom 分别控制水平/垂直缩放，**负值即翻转**：xzoom=-1 是水平镜像，常用于「角色转身面向另一边」——用一张素材左右翻转即可，省一套素材。
它也能做非等比压扁拉伸（如 xzoom=1.2, yzoom=0.8）。`,
    paramManual: [
