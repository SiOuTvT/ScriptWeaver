...
export const tfColorEnc: Encyclopedia = {
  'tf-alpha': {
    artGuide: `alpha 控制立绘整体的不透明度，淡入淡出、半透明幽灵、梦境虚化都靠它打底。但它有个坑：alpha 是「逐子图独立作用」的，多图层立绘重叠的地方可能透出下层，需要 Flatten() 来规避。绝大多数淡入/淡出演出，底层其实都是 alpha 在动。`,
    paramManual: [
...
alpha 动画建议配合 will-change: opacity 提升为合成层，避免主线程重绘。`,
  ...
  'tf-additive': {
    artGuide: `additive 把混合方程切成 ADD 加色：亮的地方越叠越亮、暗的地方近乎透明，做出霓虹、魔法辉光那种发光感。它就是「发光」的开关，魔法光效、霓虹灯、能量护盾、赛博辉光都用它。设回 0.0 就恢复普通覆盖（OVER）。`,
    paramManual: [
...
它和 alpha 通道交互，通常是半透明区域才出光晕，实心不透明图加色效果有限。`,
  ...
  'tf-nearest': {
    artGuide: `nearest 管的是纹理采样滤波：True 是最近邻（硬边像素风），False 是双线性（平滑）。像素艺术、复古风必备，默认是继承父级或者 config 的设置。它只影响「放大时怎么采样」，不改图像本身。`,
    paramManual: [
...
它是继承属性，父级设了子级默认跟随，不必每个都写。`,
  ...
  'tf-blur': {
    artGuide: `blur 是高斯模糊，失焦、出神、梦境、景深、回忆那种柔化全靠它。它会先把子图展平（flatten）到透明背景再做卷积，所以半透明的边缘也能正确糊掉。半径越大越糊，常配合 alpha 做出「渐渐朦胧」的感觉。`,
    paramManual: [
...
它会让边缘渗入透明，注意周边留白；动画 blur 频繁变化开销更高，建议离散档位而非连续。`,
  ...
  'tf-matrixcolor': {
    artGuide: `matrixcolor 是统一重着色的入口，接收一个 4×4 颜色矩阵（或者 ColorMatrix 子类的实例），矩阵之间还能相乘组合。任何统一染色、昼夜切换、去色回忆、故障滤镜的底层都在它这儿，它是颜色处理的总闸。`,
    paramManual: [
...
复杂组合用变量缓存矩阵，避免每帧重建矩阵对象（GC 压力）。`,
  ...
  'mc-brightness': {
    artGuide: `BrightnessMatrix 是整体加减亮度（不动 Alpha）：value=-1 全黑、0 不变、1 全白。关灯变黑、闪光过曝、情绪的明暗调节，用它最干脆。`,
    paramManual: [
...
它是加性运算，和对比度/饱和度组合时注意顺序（先亮度还是先对比结果不同）。`,
  ...
  'mc-contrast': {
    artGuide: `ContrastMatrix 是绕着中灰 0.5 缩放对比度：>1 把颜色往两极推（更硬朗、对比更强），<1 把色拉向中灰（更灰、发雾）。硬派战斗滤镜、老照片灰调、强化视觉冲击，用它。`,
    paramManual: [
...
和 brightness 顺序敏感，建议固定「先 brightness 后 contrast」的管线顺序。`,
  ...
  'mc-saturation': {
    artGuide: `SaturationMatrix 调的是饱和度（不动 Alpha）：1 是原色、0 是完全灰度。回忆去色、黑白闪回、想强调某色时降点饱和、情绪灰调，都靠它。去饱和时按亮度权重 (0.2126,0.7152,0.0722) 来保留灰度。`,
    paramManual: [
...
动画 saturate 很便宜，适合做「回忆渐褪」的渐变。`,
  ...
  'mc-hue': {
    artGuide: `HueMatrix 是把颜色绕色环转指定度数（不动 Alpha）：用来做「换色、异世界滤镜、情绪染色」。毒气绿、魔法粉、整体换色但不改明暗，全靠它。`,
    paramManual: [
...
和其它滤镜顺序敏感，建议放在滤镜链靠后。`,
  ...
  'mc-invert': {
    artGuide: `InvertMatrix 是反相颜色通道（不动 Alpha）：0 到 1 控制反转量，1 是完全底片效果。负片闪回、故障美学、强烈的视觉反转，用它。`,
    paramManual: [
...
光敏/不适人群慎用满反相。`,
  ...
  'mc-opacity': {
    artGuide: `OpacityMatrix 只乘算 Alpha（不动颜色），功能上和 alpha 属性一样，但它是矩阵形式，方便跟其他 ColorMatrix 组合、在一处统一管理颜色和透明变换。`,
    paramManual: [
...
filter: ... ;   /* 与 feColorMatrix 链组合 */`,
  ...
  'mc-colorize': {
    artGuide: `ColorizeMatrix 是把「黑白图像」在指定的黑、白两色之间重新染色（不动 Alpha），适合双色剪影、夜视仪绿、热成像。它先按亮度转灰，再在 black→white 之间线性上色。`,
    paramManual: [
...
纯黑/纯白区域会完全变成对应色，失去细节。`,
  ...
  'mc-tint': {
    artGuide: `TintMatrix 是给整张图蒙上一层颜色（不动 Alpha），最常用的就是「夜晚蓝、回忆黄、危险红」这种情绪统一着色。整图染上那一层，明暗关系基本保留。`,
    paramManual: [
...
染色是乘性，白色区域保留最亮，黑色区域几乎不变。`,
  ...
  'mc-sepia': {
    artGuide: `SepiaMatrix 是去饱和再加棕黄染色，一键复古老照片质感，等价于 TintMatrix("#ffeec2") 乘 SaturationMatrix(0.0)。回忆、年代感闪回的复古滤镜用它。`,
    paramManual: [
...
注意别和强 saturation 冲突（一个去色一个加色）。`,
  ...
  'mc-identity': {
    artGuide: `IdentityMatrix 是 4×4 单位阵，完全不改变任何颜色和 Alpha。它唯一的「用处」是在 ATL 颜色动画里当「无变化的起点/终点」占位，保证两端矩阵结构一致、能插值。`,
    paramManual: [],
...
简单呼吸用多关键帧 CSS 即可近似，不必上 SplineMatrix。`,
  ...
  'mc-matrix': {
    artGuide: `Matrix 是用 16 个数字直接定义 4×4 颜色变换矩阵（比如交换红绿通道）。它是颜色处理的「裸金属」，得遵守预乘 Alpha 的约定，不然缩放时会出现伪影。`,
    paramManual: [
