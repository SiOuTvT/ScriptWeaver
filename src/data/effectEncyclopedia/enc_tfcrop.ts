...
export const tfCropEnc: Encyclopedia = {
  'tf-crop-prop': {
    artGuide: `crop 是「取景框」：把立绘裁切成指定矩形 (x,y,w,h)，坐标相对子图左上角。它是做「镜头推近脸部特写」「聚焦局部」「模拟放大看某处」的关键——动画 crop 从全图 (0,0,1,1) 缩到脸部小框，即「推近特写」。`,
    paramManual: [
...
crop 框外的部分被裁，若配合 zoom 放大做「推近+放大」，注意预留透明边距防止裁到立绘主体。`,
  ...
  'tf-corner': {
    artGuide: `corner1 / corner2 用左上(corner1)与右下(corner2)两个对角点定义裁剪盒，比 crop 写四元组更直观。当你想用「两个角点」方式裁切时，它比四元组好读。
注意：若同时写 crop，crop 优先级更高。`,
    paramManual: [
...
corner 写法直观但底层仍是 crop，动画行为一致。`,
  ...
  'tf-xysize': {
    artGuide: `xysize / xsize / ysize 把立绘强制缩放到指定宽高。当你要把不同素材统一到固定尺寸、做缩略图或统一机位大小时用它。
具体如何适配（保比例还是拉伸）由 fit 决定。`,
    paramManual: [
...
响应式布局里用固定 xysize 可能在小屏溢出。`,
  ...
  'tf-fit': {
    artGuide: `fit 配合 xsize/ysize 决定缩放策略：contain（含入不超界，可能留白）、cover（覆盖不亏，可能裁切）、fill（直接拉伸填满，会变形）。
满屏背景用 cover，统一框内不溢出用 contain，故意变形艺术化用 fill。`,
    paramManual: [
...
contain 留白处会露出下层，注意背景协调。`,
  ...
  'tf-maxsize': {
    artGuide: `maxsize 把立绘缩放到「不超过该框」且保持比例（等价于 xysize + fit="contain"）。它是响应式布局里防止大图撑爆、又避免小图被放大的稳妥约束。`,
    paramManual: [
