...
export const tfPosEnc: Encyclopedia = {
  'tf-pos-prop': {
    artGuide: `pos / xpos / ypos 是立绘「落在屏幕何处」的最基础坐标。当你需要精确摆位、自定义非标准机位、或做「镜头扫到某坐标」的补间动画时，直接用坐标值最直观。
记住 position 类型的灵活性：浮点 0.0~1.0 表示「占父容器宽/高的比例」，absolute(n) 表示「绝对像素」。混用两者做补间会跳变。`,
    paramManual: [
...
补间时**不要混用**比例与 absolute，否则 Ren'Py 会强制转换导致中途跳变。`,
  ...
  'tf-align': {
    artGuide: `align / xalign / yalign 是最常用的摆位方式：它把「位置」和「锚点」设成同一个值，于是「立绘的某比例点」精确对齐「父容器的某比例点」——xalign=0.5 即水平居中，比 pos 直觉得多。
绝大多数对话立绘站位（居中、贴底、对齐某比例）都用 align 完成。`,
    paramManual: [
...
它是摆位首选，但做「绕非中心旋转」时要先想清楚锚点。`,
  ...
  'tf-anchor': {
    artGuide: `anchor / xanchor / yanchor 是立绘自身的「悬挂点」：所有 rotate、scale、position 都围绕这个点运算。anchor=(0.5,1.0) 表示以「底部中心」为基准，是立绘脚底对齐的常用设置。
当你想让立绘绕脚底旋转（而非中心）、绕某特征点缩放时，必须先改 anchor。`,
    paramManual: [
...
旋转时若 anchor 不在中心，立绘会「甩出去」，务必确认这是你要的效果。`,
  ...
  'tf-offset': {
    artGuide: `offset / xoffset / yoffset 是在已定位基础上再叠加的**像素级偏移**（正向右/下），常用于说话时立绘微微前倾、强调时的轻微位移。与 pos 不同，它不受比例缩放影响。
它的价值在于「叠加微调」：不干扰主定位（pos/anchor），又能做轻量动效。`,
    paramManual: [
...
但注意它仍受父容器裁剪影响，偏移过大会被裁切边。`,
  ...
  'tf-center': {
    artGuide: `xycenter / xcenter / ycenter 把「立绘自身中心」放到指定坐标（等价于 pos + anchor=(0.5,0.5)）。当你需要「立绘中心精准落在某点」且之后要旋转/缩放时，用它比 pos 更稳——因为中心被锚定，后续变换都绕中心。`,
    paramManual: [
...
做旋转/缩放前用 center 摆位最稳，避免「转着转着偏心」。`,
  ...
  'tf-subpixel': {
    artGuide: `subpixel 是「亚像素定位」开关：开启后用小数像素精度绘制，慢速移动时边缘更顺滑、不跳格。任何「不想看到像素一格一格跳」的位移动画（细线移动、慢速平移）都应开它。
代价是需预留透明边距，否则旋转/放大会被裁剪盒切边。`,
    paramManual: [
...
对静态不动物体开 subpixel 毫无收益，只在位移动画时有意义。`,
  ...
  'tf-polar': {
    artGuide: `around / angle / radius 用「起点 + 角度 + 半径」表达位置，特别适合圆周运动——让立绘绕某点公转一圈，只需插值 angle。angle 0°指正上、90°指正右，引擎自动归一到 0~360。
行星公转、圆周入场、指针/罗盘类动画都离不开它。`,
    paramManual: [
...
但 angle 与直角 pos 混用做补间会跳变，整段动画要统一坐标系。`,
  ...
  'tf-polar-anchor': {
    artGuide: `anchoraround / anchorangle / anchorradius 把极坐标机制搬到「锚点」上：让立绘的锚点绕某圆心做圆周运动。于是立绘「公转」时，自身悬挂点也跟着转——适合卫星绕行星（卫星始终某点朝外）、绕轴旋转的挂件。`,
    paramManual: [
