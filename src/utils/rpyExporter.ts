...
/**
 * 生成 ui.rpy：覆盖主菜单，提供标题画面(封面)。
 * 仅当设置了封面背景图时生成；以官方 screen main_menu 为骨架，
 * 加入背景封面图 + 开始 / 读取 / 设置 / 关于 / 退出 按钮，
 * 保证打开导出的游戏即有封面，而非默认 "Ren'Py" 空菜单。
 */
export function exportTitleScreenRpy(meta: ProjectMeta, assets: AssetItem[]): string {
  if (!meta.coverAssetId) return ''
  ...
  return [
    '# Generated by ScriptWeaver — 标题画面(封面) (ui.rpy)',
    'screen main_menu():',
    '    tag menu',
    `    add "${img}"`,
    '    vbox:',
