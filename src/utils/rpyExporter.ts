...
function compileToNodes(
  ...
  for (let i = 0; i < resolvedStates.length; i++) {
    ...
    // ---- 存档点（在台词/选择支后自动存档）----
    if (delta.save_point) {
      block.push({ kind: 'python', expr: 'renpy.autosave()' })
    }
  ...
  // 全局结局画面：在最终 return 之前插入结局展示（show screen → pause → hide → return），
  // 仅当启用时。这样剧情跑完后先定格展示「The End」再退回主菜单。
  if (endingEnabled) {
    if (nodes.length && nodes[nodes.length - 1].kind === 'return') nodes.pop()
    nodes.push({ kind: 'ending' })
    nodes.push({ kind: 'return' })
  }
  return nodes
}
...
function serializeNodes(nodes: RpyNode[], totalLines: number): string {
  ...
  for (const n of nodes) {
    ...
    if (n.kind === 'ending') {
      lines.push('    show screen the_end')
      lines.push('    pause')
      lines.push('    hide screen the_end')
      continue
    }
...
/**
 * 生成 options.rpy：窗口标题 / 关于页名 / 打包名。
 * 对齐官方 Building Distributions 与 config 文档（build.name 不能含空格/冒号/分号）。
 */
export function exportOptionsRpy(meta: ProjectMeta): string {
  const title = (meta.title || 'My Visual Novel').trim() || 'My Visual Novel'
  const safe = title.replace(/[^\w一-龥-]/g, '_').slice(0, 60) || 'scriptweaver_project'
  const textSpeed = meta.textSpeed ?? 30
  const afm = meta.autoForwardDelay ?? 15
  const mus = meta.musicVolume ?? 100
  const sfx = meta.soundVolume ?? 100
  const vo = meta.voiceVolume ?? 100
  const skipUnseen = meta.skipUnseen !== false
  const skipChoices = meta.skipAfterChoices !== false

  const lines = [
    '# Generated by ScriptWeaver — 游戏配置 (options.rpy)',
    '',
    'init python:',
    ...
    `    build.name = "${safe}"`,
    '',
    '    ## 文字显示速度（字符/秒，0 = 立即显示全部）',
    `    config.default_text_cps = ${textSpeed}`,
    '',
    '    ## 自动前进延迟（秒）',
    `    config.default_afm_time = ${afm}`,
    '',
    '    ## 跳过设置（注意：skip 相关是偏好设置 _preferences，不是 config 变量）',
    `    config.allow_skipping = True`,
    `    config.fast_skipping = True`,
    `    _preferences.skip_unseen = ${skipUnseen ? 'True' : 'False'}`,
    `    _preferences.skip_after_choices = ${skipChoices ? 'True' : 'False'}`,
    '',
    '    ## 存档设置',
    ...
    '',
    '    ## 音效管理器（应用于 mix 音量）',
    `    if not hasattr(renpy.game.preferences, '_sw_volumes_set'):`,
    `        renpy.game.preferences._sw_volumes_set = True`,
    `        _preferences.set_volume("music", ${mus / 100})`,
    `        _preferences.set_volume("sfx", ${sfx / 100})`,
    `        _preferences.set_volume("voice", ${vo / 100})`,
    '',
  ]

  // CG 画廊：如果启用且有条目，生成 gallery screen
  if (meta.galleryEnabled && meta.galleryItems && meta.galleryItems.length > 0) {
    lines.push(
      '',
      '## 画廊系统 (CG Gallery)',
      'init python:',
      '    config.gallery = Gallery()',
      ...meta.galleryItems.map((item) => {
        const btn = `g.button("${item.unlockLabel || item.label}")`
        const disp = item.unlockLabel
          ? `g.unlock_image("${item.assetId}")`
          : `g.image("${item.assetId}")`
        return [
          `    g = Gallery()`,
          `    ${btn}`,
          `    ${disp}`,
          `    config.gallery.transition = dissolve`,
        ].join('\n')
      }),
      '',
      'screen gallery():',
      ...
      '        spacing 20',
      '        grid 3 2:',
      '            for item in config.gallery.buttons:',
      '                add config.gallery.make_button(item, "gallery_nav")',
...
/**
 * 生成 ui.rpy：覆盖主菜单(标题画面/封面) 与 结局画面。
 * - 封面：仅当设置了封面背景图时生成，以官方 screen main_menu 为骨架，
 *   加入背景封面图 + 开始 / 读取 / 设置 / 关于 / 退出 按钮。
 * - 结局画面：启用时生成 screen the_end()，剧情跑完返回主菜单前定格展示。
 * 两者任一存在即生成 ui.rpy（主进程按 bundle.ui 非空落盘）。
 */
export function exportTitleScreenRpy(meta: ProjectMeta, assets: AssetItem[]): string {
  const blocks: string[] = []

  // ---------- 标题画面(封面) ----------
  if (meta.coverAssetId) {
    const cover = assets.find((a) => a.id === meta.coverAssetId)
    if (cover) {
      // 封面背景在 assets 中应为 background 类型，导出到 images/background/，与 buildAssetRefs 映射一致
      const refs = buildAssetRefs(assets)
      const ref = refs.find((r) => r.sourceRelativePath === cover.relativePath)
      const img = ref ? ref.exportRelPath : `images/background/${cover.fileName}`
      blocks.push(
        [
          '# Generated by ScriptWeaver — 标题画面(封面)',
          'screen main_menu():',
          '    tag menu',
          `    add "${img}"`,
          '    vbox:',
  ...
  // ---------- 结局画面 ----------
  if (meta.endingEnabled) {
    const text = (meta.endingText || 'The End').trim() || 'The End'
    const lines = [
      '# Generated by ScriptWeaver — 结局画面',
      'screen the_end():',
    ]
    if (meta.endingAssetId) {
      const bg = assets.find((a) => a.id === meta.endingAssetId)
      if (bg) {
        const refs = buildAssetRefs(assets)
        const ref = refs.find((r) => r.sourceRelativePath === bg.relativePath)
        const img = ref ? ref.exportRelPath : `images/background/${bg.fileName}`
        lines.push(`    add "${img}"`)
      }
    }
    lines.push(`    text "${escapeDialogue(text)}":`)
    lines.push('        xalign 0.5')
    lines.push('        yalign 0.5')
    lines.push('        size 56')
    lines.push('        color "#ffffff"')
    lines.push('        outlines [ (2, "#000000", 0, 0) ]')
    lines.push('')
...
export function buildBundle(
  ...
  // 游戏元信息：标题 / 封面 / 图标 / 结局画面（需在生成 script 前确定，结局注入依赖 endingEnabled）
  const meta = projectMeta ?? { title: 'My Visual Novel' }
  const script = deltas.length === 0 ? '# No content\n' : serializeNodes(compileToNodes(deltas, resolvedStates, st, scriptLabel, used, meta.endingEnabled), deltas.length)
  ...
  const options = exportOptionsRpy(meta)
  const ui = exportTitleScreenRpy(meta, assets)
  const iconAsset = meta.iconAssetId ? assets.find((a) => a.id === meta.iconAssetId) : undefined
...
/**
 * 将 menu 节点序列化为翻译块：选项标题与提示以 `old --> new` 形式输出（new 暂置原文），
 * 跳转 / 内联变量操作结构原样保留，保证与 script.rpy 的结构一致、Ren'Py 能精确匹配。
 */
function serializeTranslatedMenu(n: Extract<RpyNode, { kind: 'menu' }>): string {
  const inner: string[] = []
  if (n.prompt) {
    const p = escapeDialogue(n.prompt)
    inner.push(`        "${p}" --> "${p}"`)
  }
  for (const ch of n.choices) {
    const cond = ch.condition ? ` if ${ch.condition}` : ''
    const t = escapeDialogue(ch.text)
    inner.push(`        "${t}" --> "${t}"${cond}:`)
    const ops = ch.ops ?? []
    for (const op of ops) inner.push(`            $ ${op}`)
    if (ch.target_label) {
...
/**
 * 生成 Ren'Py 翻译骨架（tl/<lang>/script.rpy）：
 * 复用与 script.rpy 完全相同的 AST，抽取所有台词(say)与选择支(menu)原文，
 * 按 `old --> new` 形式输出（new 暂置为原文，译者可就地替换）。
 * 锚定到对应 label，左侧 old 与 script.rpy 逐字符一致，保证 Ren'Py 精确匹配。
 * 不依赖本机 SDK：产物是标准 .rpy，直接放入 game/tl/<lang>/ 即可被 Ren'Py 加载。
 */
export function buildTranslationBundle(
  ...
  if (deltas.length === 0) return '# No translatable strings\n'
  const st = buildSymbolTable(deltas, characterConfigs, assets, positionSlots)
  const used = collectUsedTransitions(deltas, resolvedStates)
  const nodes = compileToNodes(deltas, resolvedStates, st, scriptLabel, used)

  const lang = targetLang.trim() || 'chinese'
  const blocks: string[] = []
  ...
  const flush = () => {
    if (currentLabel && currentLines.length > 0) {
      blocks.push(`translate ${lang} ${currentLabel}:`)
      blocks.push('    # 将每行右侧（new）替换为对应语言译文，左侧（old）保持原文勿改')
      for (const l of currentLines) blocks.push(l)
      blocks.push('')
    }
    currentLines = []
  }
  ...
  for (const n of nodes) {
    ...
    if (n.kind === 'say') {
      const text = escapeDialogue(n.text)
      const stmt = n.speaker ? `${n.speaker} "${text}"` : `"${text}"`
      currentLines.push(`    ${stmt} --> "${text}"`)
    } else if (n.kind === 'menu') {
      currentLines.push(serializeTranslatedMenu(n))
    }
  }
  flush()

  if (blocks.length === 0) return '# No translatable strings\n'

  const header = [
    '# Generated by ScriptWeaver',
    `# Translation language: ${lang}`,
    `# 该文件应放置于 game/tl/${lang}/script.rpy`,
    '',
  ]
  return header.join('\n') + blocks.join('\n')
}

/** 浏览器/Electron 通用：生成翻译骨架并触发下载（文件名 script_<lang>.rpy） */
export function downloadTranslation(
  ...
  const lang = (targetLang.trim() || 'chinese')
  triggerDownload(content, `script_${lang}.rpy`)
}
...
export async function exportProjectPackage(
  ...
  const bundle = buildBundle(deltas, resolvedStates, characterConfigs, assets, positionSlots, scriptLabel, variables, projectMeta)
  ...
  if (bundle.options) triggerDownload(bundle.options, 'options.rpy')
  if (bundle.ui) triggerDownload(bundle.ui, 'ui.rpy')
  return {
    mode: 'web',
    success: true,
    message: '已通过浏览器下载 script.rpy、definitions.rpy、transforms.rpy、options.rpy 与 ui.rpy（素材需手动放入 game/ 目录）。',
  }
}
